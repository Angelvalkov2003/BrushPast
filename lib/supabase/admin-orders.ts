...
import type { AdminOrder, AdminOrderDetail, OrderStatus, PaymentStatus } from "lib/types/admin";
...
export async function getOrderByIdAdmin(id: string): Promise<AdminOrderDetail | null> {
  ...
  return { ...(order as AdminOrder), ...(order as object), items: items ?? [] } as AdminOrderDetail;
}

export async function updateOrderStatusAdmin(id: string, order_status: OrderStatus) {
  const supabase = getSupabaseServiceClient();
  const { data, error } = await supabase
    .from("orders")
    .update({ order_status })
...
export async function getAdminDashboardStats() {
  ...
  const { data: orders, error } = await supabase
    .from("orders")
    .select("payment_status, payment_method, order_status, subtotal, shipping_total, grand_total, created_at");
  ...
  const countsTowardWeekRevenue = (o: (typeof list)[number]) => {
    if (o.order_status === "cancelled" || o.order_status === "refunded") return false;
    ...
    // Pay on delivery stays payment_status=pending until delivered; still counts as product revenue.
    if (o.payment_method === "cash_on_delivery") return true;
    return false;
  };
  ...
  return {
    ...
    weekRevenue: list.filter(countsTowardWeekRevenue).reduce((sum, o) => sum + productTotal(o), 0),
  };
}
