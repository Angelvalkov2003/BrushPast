...
import type { LittleGeorgeParagraph } from "lib/stories/little-george-content";
...
function StoryParagraph({ block }: { block: LittleGeorgeParagraph }) {
  if (block.pull) {
    return (
      <blockquote
...
export async function LittleGeorgePage() {
      ...
      <RevealSection className="border-b border-bp-text/10">
        <div className="mx-auto grid max-w-[1400px] lg:grid-cols-2 lg:items-stretch">
          <div className="flex flex-col justify-center px-4 py-10 md:px-10 md:py-14 lg:py-16">
            ...
            <h1 className="mt-2 text-[clamp(2.75rem,10vw,5.5rem)] uppercase leading-[0.88] tracking-tighter text-bp-text">
              {COPY.title}
            </h1>
          ...
          <div className="relative flex items-center justify-center bg-[#ebe3d8] px-4 py-8 md:px-10 lg:py-12">
            <div
              ...
              style={{
                clipPath: "polygon(4% 2%, 100% 0%, 98% 100%, 0% 97%, 3% 70%, 1% 38%)",
              }}
            >
              <Image
                src={heroImage}
