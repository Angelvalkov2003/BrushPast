...
function resolveWays(categories: ShopCategory[]) {
  return HOME_SHOP_WAYS.map((fallback) => {
    ...
    return {
      ...
      image:
        displayImageUrl(cat?.image_url) ??
        displayImageUrl(fallback.image) ??
        null,
      icon: iconKey
        ? brushPastIcons.homepage[iconKey]
        : brushPastIcons.homepage.wearIt,
    };
  });
}
              ...
              >
                ...
                >
                  <PolaroidFrame index={index + 1}>
                    <div className="relative aspect-[4/5] overflow-hidden bg-bp-text/5">
                      {displayImageUrl(way.image) ? (
                        ...
                        <div className="flex h-full flex-col items-center justify-center gap-3">
                          ...
                          <span
                            className={`${bpWhisperUtility} text-xl text-bp-text/50`}
                          >
                            {way.title}
                          </span>
                        </div>
                    ...
                    >
                      <WayIcon
                        className="h-7 w-7 text-bp-accent md:h-8 md:w-8"
                        strokeWidth={1.5}
                        aria-hidden
                      />
                      {way.title}
                  ...
                  <p
                    className={`${bpBodyClass} mt-4 text-center text-bp-text/75`}
                  >
                    {way.description}
                  </p>
                  <p
                    className={`${bpBodyClass} ${bpLinkUtility} mt-3 text-center text-bp-accent opacity-0 transition-opacity group-hover:opacity-100`}
                  >
                    {way.cta} →
                  </p>
                </Link>
